...
async function getRpi7in3f(orientation?: Orientation, colorMode?: ColorMode): Promise<DisplayDevice> {
    try {
        const { Rpi7in3f } = await import('@epaperjs/rpi-7in3f');
        return new Rpi7in3f(orientation, colorMode);
    } catch (e) {
        throw new Error('Failed to import @epaperjs/rpi-7in3f, make sure it is installed');
    }
}